...
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NOT NULL; -- NULL 허용 O

--ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NULL; -- NULL 허용 X
...
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NULL; -- NULL 허용 O
...
-- SQL Error [1441] [72000]: ORA-01441: 일부 값이 너무 커서 열 길이를 줄일 수 없음 에러남
...
-- 이미 값이 들어있어서 안바뀜. 이제 들어가는 거 부터 바뀔듯 NULL 인애들만 바뀜
...
-- 테이블은 행과 열을 가지는 DB 객체로 최소 한 개 이상의 컬럼을 가져야 한다!!!
-- 모든 컬럼을 삭제하려고 하면 에러남
...
-- DDL(CREATE)이 트랜잭션을 밀어서 DB에 함께 반영!!
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT DEPT_COPY_PK  TO PKPKPK;
