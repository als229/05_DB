...
-- 2) DEPT_CODE 가 같은 행을 그룹으로 만들어라
-- 3) 묶은 그룹의 DEPT_CODE와 각 그룹의 속한 인원의 급여 합을 조회
...
SELECT DEPT_CODE ,FLOOR(AVG(SALARY)) AS "평균 급여"  -- (4) 그룹별 평균 조회 
FROM EMPLOYEE -- (1) EMPLOYEE 테이블 선택 
WHERE DEPT_CODE IN ('D5', 'D6')  -- (2) 사용할 행 선택 
GROUP BY DEPT_CODE ; -- (3) 선택된 9개의 행에서 DEPT_CODE로 그룹 나눔 

-- EMPLOYEE 테이블에서 직급 별 2010년도 이후 입사자들의 급여 합을 조회
SELECT JOB_CODE ,SUM(SALARY) AS "직급 별 급여 합" 
FROM EMPLOYEE 
...
--   =>GROUP BY 를 통해서 만들어진 그룹 중 조건에 맞는 그룹만 선택

-- HAVING 컬럼명 | 그룹 함수식 비교연산자 비교값

-- 부서별 평균 급여가 3,500,000원 이상인 부서를 조회하여 부서코드 오름차순으로 정렬

/* 1) WHERE 절에 조건 설정 */

/*4*/ SELECT DEPT_CODE, FLOOR(AVG(SALARY)) 
/*1*/ FROM EMPLOYEE 
/*2*/ WHERE  SALARY >= 3500000
/*3*/ GROUP BY DEPT_CODE  
/*5*/ ORDER BY DEPT_CODE ASC;
--> 개인 급여가 350만 이상인 사람들만 모아서 부서별로 그룹을 나누고 평균을 구한 식
--> 요구사항과 다름. 요구 사항은 부서별 평균 급여가 350만 이상
...
ORDER BY DEPT_CODE ASC;
--> 나눠진 부서 그룹별로 급여 평균을 구했을 때 급여 평균이 350만 이상인 부서만 조회
...
-- WHERE 절에 BONUS IS NOT NULL 사용시
SELECT 
	JOB_CODE 
FROM EMPLOYEE 
