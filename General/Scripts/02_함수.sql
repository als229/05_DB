...
ORDER BY "이메일 길이" ASC; -- 16행 조회

--------------------------------------------------------------------------------------------------

/* INSTR(문자열 | 컬럼값, '찾을 문자열' [, 시작 위치 [, 순번]]) []는 생략가능한 부분
 * - 시작 위치부터 지정된 순번까지 문자열 | 컬럼 값에서 '찾을 문자열'의 위치를 반환 
 */

-- 처음부터 찾아서 처음 찾은 B의 위치 조회
SELECT 'AABBCCABC', INSTR('AABBCCABC', 'B') 
FROM DUAL;

-- 5번째 글자 부터 찾아서 처음 찾은 B의 위치 조회
SELECT 'AABBCCABC', INSTR('AABBCCABC', 'B', 5) 
FROM DUAL;

-- 5번째 글자 부터 찾아서 3번째로 찾은 C의 위치 조회
 ...
 * 
 * - ROUND(실수[, 소수점 위치]) : 반올림
 * 	1) 소수점 위치 X : 소수점 첫째 자리에서 반올림 -> 정수 형태로 반환
 * 	2) 소수점 위치 O : 지정된 위치가 표현 되도록 반올림
 * 	EX) ROUND(3.14, 1) => 3.1 , ROUND(3.141592, 2) => 3.14
 *  
 * - TRUNC(실수 [, 소수점 위치]) : 버림
 */
 ...
 */
SELECT
	CEIL(MONTHS_BETWEEN(TO_DATE('2025-07-17', 'YYYY-MM-DD'),CURRENT_DATE)) -- 5 
FROM DUAL;

/* 나이 구하기 */
-- 2001.03.20 생의 나이, 만 나이 구하기
SELECT 
	CEIL ((SYSDATE - TO_DATE('1994.03.20', 'YYYY.MM.DD')) / 365 +1) AS "한국식 나이" -- 윤년 계산 없음
FROM DUAL;

SELECT 
	FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('1994.07.05', 'YYYY.MM.DD')) / 12) -- 윤년 계산도 있음 정확함
FROM DUAL;
...
/* ADD_MONTH(날짜, 숫자) :
 * - 날짜에 숫자 만큼의 개월 수 추가
 * - 달 마다 일 수가 다르기 때문에 계산이 쉽지 않음 => 쉽게 계산할 수 있는 함수 제공
 */
SELECT 
	SYSDATE + 28, 					-- 3/20
...
-- '.', '-', '/' : 등의 날짜 표시 기호와 기본적인 날짜 작성 순서는 포맷 지정을 안해도 해석 가능하다.
SELECT 
'2025-02-20' AS 문자열
...
SELECT 
	...
	, TO_DATE('2025-02-21 17:50:00 (금)','YYYY-MM-DD HH24:MI:SS (DY)') AS DATE타입
FROM DUAL;

SELECT 
'16:20:43 목요일 02-20/2025'
, TO_DATE('16:20:43 목요일 02-20/2025', 'HH24:MI:SS DAY MM-DD/YYYY') 
FROM DUAL;
...
SELECT 
	...
	, TO_DATE('2025년 02월 20일', 'YYYY"년" MM"월" DD"일"') 
FROM DUAL;
...
-- 문자열 9칸 지정, 오른쪽 정렬
SELECT 1234, TO_CHAR(1234, '9999999999999') FROM DUAL; 

-- 문자열 9칸 지정, 오른쪽 정렬, 빈칸을 0으로 채움
SELECT 1234, TO_CHAR(1234, '0000000000000') FROM DUAL; 
...
-- , 를 이용한 자릿수 구분 + 화폐기호
SELECT 
	100000000, TO_CHAR(100000000,'999,999,999') 
	FROM DUAL;

-- ￦붙음 앞에
SELECT 
	100000000, TO_CHAR(100000000,'L999,999,999') 
	FROM DUAL;

-- 앞에 공간 많아서 잘라줌
SELECT 
	100000000, TRIM(TO_CHAR(100000000,'L999,999,999')) 
	FROM DUAL;
...
-- 13 곱하면 순서가 이상해짐 왜? 지금은 연봉으로 비교하는데 연봉은 지금 문자열이라 정확한 비교가 안됨
SELECT 
	EMP_NAME 
	,TRIM(TO_CHAR(SALARY * 13, 'L999,999,999')) AS 연봉  
FROM EMPLOYEE 
ORDER BY 연봉 DESC;
...
-- 오늘 날짜를 '2025년 02월 20일 (목) 오후 04시 30분 20초'
SELECT
	SYSDATE 
	,TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" (DY) AM HH"시" MI"분" SS"초"') 
FROM DUAL;
 ...
 * 
 * RR : 50을 기준으로
 * 			50 미만이면 현재 세기로 표기 (2000년대)
 * 			50 이상이면 이전 세기로 표기 (1900년대)
 */
...
-- 차이 0 (50년을 기준으로 차이점이 보임)
SELECT
	TO_DATE('50/02/20', 'YY/MM/DD') AS YY   -- 2025
	,TO_DATE('50/02/20', 'RR/MM/DD') AS RR  -- 2025
FROM DUAL;
...
SELECT 
	'$1,500', TO_NUMBER('$1,500','$9,999') AS 숫자 
FROM DUAL;
...
SELECT EMP_ID ,EMP_NAME,PHONE, NVL(PHONE,'없음') AS PHONE 
FROM EMPLOYEE; 
...
SELECT EMP_NAME ,SALARY ,BONUS , SALARY  * NVL(BONUS , 0)  
FROM EMPLOYEE; 
...
SELECT EMP_ID ,EMP_NAME ,NVL2(PHONE,RPAD(SUBSTR(PHONE,1,3),LENGTH(PHONE),'*' )  , '없음') ,
FROM EMPLOYEE ;

-- RPAD(문자열, 길이, 바꿀 문자)
--> 문자열 전체에서 오른쪽을 지정된 길이 만큼 다른 바꿀문자로 변경
...
/* DECODE(컬럼명, 조건1, 결과1, 조건2, 결과2, ....[, 아무것도 만족X])
 * 
 * - 컬럼 값이 일치하는 조건이 있으면 해당 조건 오른쪽에 작성된 결과가 반환되는 함수
 * 
 * - SWITCH, CASE, DEFAULT 와 비슷
 */
...
SELECT EMP_NAME , JOB_CODE , SALARY, DECODE(JOB_CODE, 'J7', SALARY*1.2, 'J6', SALARY*1.15, 'J5', SALARY*1.1, SALARY*1.05) AS "인상된 급여"
FROM EMPLOYEE 
...
/* 그룹 함수 */
/* - N개의 행의 컬럼 값을 함수로 전달하였을 때 결과가 1개만 반환되는 함수
 *  
 ...
 * 
 * 1) COUNT(*) 
 *  - 조회된 모든 행의 개수를 반환 (NULL이 포함된 행도 개수 증가)
 * 
 * 2) COUNT(컬럼명) 
 *  - 지정된 컬럼 값이 NULL인 행을 제외하고 나머지 행의 개수 반환
 * 
...
SELECT COUNT(DISTINCT DEPT_CODE) -- 6 (NULL 제외)
FROM EMPLOYEE ;
...
SELECT 
	(SELECT COUNT(*) FROM EMPLOYEE WHERE SUBSTR( EMP_NO, 8,1) = 1) AS 남자
	, (SELECT COUNT(*) FROM EMPLOYEE WHERE SUBSTR( EMP_NO, 8,1) = 2) AS 여자
FROM DUAL ;
