...
INSERT INTO V_DCOPY2
VALUES ('A1', '마케팅1팀');
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH02_KKM"."DEPT_COPY2"."LOCATION_ID") 안에 삽입할 수 없습니다
-- 에러발생 NULL

-- V_DCOPY2를 이용하면 LOCATION_ID 컬럼에 INSERT 할 수 없기 때문에
-- 자동으로 NULL이 삽입됨
--> 하지만 LOCATION_ID는 NOT NULL 제약조건이 있기 때문에 NULL 삽입 불가 에러뜸.

--> 테이블, VEIW 구조를 파악하지 못하고 있으면 VIEW를 이용한 DML 사용은 힘들다.
...
NOCYCLE				-- 순환X
NOCACHE				-- 미리 만들어 두는 시퀀스 번호 없음
; 
-- 캐시 : 프로그램에 필요한 정보들을 임시 저장해 놓는 공간
...
-- => NEXTVAL 가 한 번도 호출된 적이 없다.
-- => NEXTVAL 한번도 안 쓰고 확인하면 에러남
...
-- 150 이상으로 증가 불가 => MAX VALUE 150이라서

--------------------------------

-- SEQUENCE 변경(ALTER)

-- CREATE SEQUENCE 와 작성법이 유사함
--> 단, START WITH 제외
...
-- TEST_NO(PK,여기에 INDEX 있음), TEST_ID (NOT NULL)
 ...
 *  */
SELECT * FROM TB_IDX_TEST 
WHERE TEST_ID = 'TEST500000'; -- 평균 0.03 정도 나옴

SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO = 500000; -- 평균 0.02 정도 나옴
