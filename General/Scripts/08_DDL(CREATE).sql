 ...
 * 이름					: MEMBER_NAME				VARCHAR2(15) => 한글 5글자 까지
 * 주민등록번호	: MEMBER_SSN				CHAR(14)		 => '-' 까지 포함
...
COMMENT ON COLUMN "MEMBER".MEMBER_SSN IS '회원 주민등록 번호';
...
-- DESC문 : 테이블의 구조를 표시
DESC MEMBER;
--> 명령 프로토콜에서 쓰는 놈 여기서는 못 씀

-- MEMBER 테이블에 샘플 데이터 삽입
INSERT INTO "MEMBER" 
VALUES('MEM01', '123ABC', '홍길동', '999999-9999999', SYSDATE);
...
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" 
VALUES('MEM02', 'ABC123', '김미영', '999999-1111111', SYSDATE);


-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" 
VALUES('MEM03', 'ABC12356', '엄현우', '999999-2222222',DEFAULT);


-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO "MEMBER" (MEMBER_ID, MEMBER_PW, MEMBER_NAME,MEMBER_SSN)
VALUES('MEM04', '1Q2W3E4R', '오세현', '999999-3333333');
...
/* CREATE TABLE 수행 시 제양조건을 설정하는 방법
 * 
...
/* NOT NULL 제약조건은 "컬럼 레발" 로만 설정이 가능하다!!
 */
...
-- NOT NULL + UNIQUE 제약조건 특징이 합쳐져 있음
...
CREATE TABLE USER_USED_PK(
    ...
    -- 테이블 레벨
    -- , CONSTRAINT SUER_NO_PK PRIMARY KEY (USER_NO)
);
...
--> FOREIGN KEY가 설정된 컬럼의 값은 외부(다른 테이블) 테이블의 값을 저장할 수 있다.(== 다른 테이블에서 사용할 수 있다(참조)).
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER, -- 10, 20, 30, NULL 까지만 들어갈 수 있다.
...
--SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(KH02_KKM.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
-- SQL Error [2292] [23000]: ORA-02292: 무결성 제약조건(KH02_KKM.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
-- 에러남
...
CREATE TABLE USER_USED_FK2(
  ...
  -- 컬럼 레벨 설정
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK2 REFERENCES USER_GRADE2(GRADE_CODE)
  ON DELETE SET NULL
...
CREATE TABLE USER_USED_FK3(
  ...
  -- 테이블 레벨
  --CONSTRAINT GRADE_CODE_FK3  => 제약조건명 설정(생략가능)
  FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE3(GRADE_CODE)
  ON DELETE CASCADE -- 부모행 참조하는 모든 자식행 삭제
);
...
SELECT * FROM USER_GRADE3; -- 2행 -- 10 삭제
SELECT * FROM USER_USED_FK3; -- 2행 -- 10을 참조하던 홍길동, 이순신 삭제됨

/* FK 제약조건을 이용하면 테이블간의 관계가 형성된다!!
 * => 참조를 당하는 부모 테이블의 컬럼과
 * -- 참조를 하는 자식 테이블의 컬럼 값이 같은 형태를 띄게 된다!!
...
SELECT *
FROM USER_USED_FK F
LEFT JOIN USER_GRADE G ON (F.GRADE_CODE = G.GRADE_CODE);
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH02_KKM.GENDER_CHECK)이 위배되었습니다
-- GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 기록 가능한데 '남자'라는 조건 이외의 값이 들어와 에러 발생

/* CHECK 제약조건은 설정이 간편하지만
 * 조건을 수정하는데 어려움이 있다!!
 * => CONSTRAINT는 수정할 수 없다. 삭제하고 다시 넣어야함.
 * 
 * => 그래서 조건이 다양한 경우에는 FK 제약조건을 이용하는 편이 더 좋다.
 */
...
INSERT INTO CHECK_TEST VALUES ('짱아', 90); 
INSERT INTO CHECK_TEST VALUES ('유리', 100); 
INSERT INTO CHECK_TEST VALUES ('맹구', 300); 
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH02_KKM.CHECK_SCORE)이 위배되었습니다
-- 범위로도 잘 작동
 ----------------------------------------------------------------------------------------------------------------
...
--> 서브쿼리로 테이블 생성 시 별도의 제약조건 설정, DEFAULT 설정, COMMENT 설정 등이 필요!
 ...
 * 
 * NOT NULL은
 * 컬럼의 상태(NULL 허용, NULL 허용 불가)를 바꾸는(MODIFY) 모습
 * 
 * */
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT DEPT_CODE_ID2 FOREIGN KEY (DEPT_CODE) REFERENCES DEPARTMENT(DEPT_ID);  
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT SAL_LEVEL_FK FOREIGN KEY (SAL_LEVEL) REFERENCES SAL_GRADE(SAL_LEVEL);  


-- DEPARTMENT테이블의 LOCATION_ID에 외래키 제약조건 추가
-- 참조 테이블은 LOCATION, 참조 컬럼은 LOCATION의 기본키
ALTER TABLE DEPARTMENT ADD CONSTRAINT LOCATION_ID_FK FOREIGN KEY (LOCATION_ID) REFERENCES LOCATION(LOCAL_CODE);  


-- LOCATION테이블의 NATIONAL_CODE에 외래키 제약조건 추가
-- 참조 테이블은 NATIONAL, 참조 컬럼은 NATIONAL의 기본키
ALTER TABLE LOCATION ADD CONSTRAINT LOCATION_CODE_FK FOREIGN KEY (NATIONAL_CODE) REFERENCES NATIONAL(NATIONAL_CODE);  
